...
class RsyncGUI(tk.Tk):
    ...
    def refresh_ips(self):
        """
        Atualiza IPs; pinta e marca automaticamente se detectado.
        Não desmarca automaticamente para não desfazer seleção manual.
        """
        ...
        for name, info in self.devices.items():
            ...
            if ip:
                ...
                # seleciona automaticamente se foi detectado (não altera seleção manual pré-existente)
                if info["var"].get() == 0:
                    info["var"].set(1)
            else:
                info["ip_label"].config(text="não encontrado", fg="white")
                info["cb"].configure(style="Default.TCheckbutton")
                # se quiser desmarcar quando não encontrado, descomente abaixo:
                # info["var"].set(0)
